...
def combine_bd_financials(UPDATED=False):
    ...
    """
    # Load historical data (up to config.END_DATE) from local file or fetch if necessary
...
def prep_dataset(dataset, UPDATED=False):
    """
    Prepare dataset by dropping duplicates, converting quarter to date, 
...
def convert_ratios_to_factors(data):
    ...
    # AR(1) with constant for book capital ratio
...
def macro_variables(db, UPDATED=False):
    ...
    Note: The function starts gathering data from one year earlier than 1970 to allow for factor calculation.
    """
...
def convert_and_export_tables_to_latex(corrA, corrB, UPDATED=False):
    ...
    # Define the column format and header row manually
