...
from load_fred import *  # 请确保此模块在你的项目中可用

"""
Functions that pull and prepare the data for Table 03.
"""

# 使用 config 中的 DATA_DIR 作为基准目录
...
def fetch_data_for_tickers(ticks, db):
    ...
    Parameters:
      ticks (DataFrame): DataFrame containing ticker information including 'gvkey', 'Start Date', 'End Date'，
                           以及可选的 'Ticker' 列。
      db: WRDS 数据库连接对象。
...
def load_bd_financials():
    ...
    Returns:
      bd_financials (DataFrame): DataFrame containing financial assets and liabilities of security brokers and dealers.
    """
...
def load_fred_past(url=URL_FRED_2013, data_dir=DATA_DIR, prn_file_name='ltab127d.prn', csv_file_name='fred_bd_aem.csv'):
    ...
    Parameters:
      ...
      data_dir (Path): Directory where the output CSV file should be saved.
