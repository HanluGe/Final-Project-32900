...
以及可选的 LaTeX 文档编译（若存在 combined_document.tex）。
"""
...
def task_compile_latex_docs():
    ...
    如果 config.OUTPUT_DIR 下不存在 combined_document.tex，则跳过此任务。
    """
    tex_directory = Path(config.OUTPUT_DIR)
    tex_file = tex_directory / "combined_document.tex"
    target_pdf = tex_directory / "report_final.pdf"

    if not tex_file.exists():
        print("combined_document.tex 不存在，跳过 LaTeX 编译任务。")
        return None

    compile_command = ["latexmk", "-pdf", "-pdflatex=xelatex", str(tex_file)]
